...
- #### Response:
  - **Success** (`201`): JSON object with details of the newly created FD record, including `FD_ID`, `Branch_ID`, `Account_ID`, `Period`, `StartDate`, and `InitialAmount`.
  - **Error** (`500`): JSON object with error message.
...
### 4. Update FD by ID
- **Endpoint**: PUT `https://ourbankURL/api/fixedDeposits/:id`
- **Description**: This API updates an existing FD record by its ID.
#### Path Parameter:
  - `id` (required): The unique ID of the FD to update.
#### Request:
- **Request Body** (JSON): JSON object containing the fields to update 
#### Response:
- **Success** (`200`): JSON message indicating "FD updated successfully."
- **Not Found** (`404`): JSON message indicating "FD not found."
- **Error** (`500`): JSON object with error message.
